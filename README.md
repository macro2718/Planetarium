...
## テスト
- 天文計算、時間制御、画面ルーティング、設定レジストリ、図書室ストア、モジュール依存を検証します。
  ```bash
  npm test
  ```
- 全 JavaScript ファイルの構文チェック:
  ```bash
  npm run check
  ```

## ディレクトリの目安
- `planetarium.js`: コアと UI の依存を組み立てるアプリケーションのエントリーポイント。
- `astroCatalog.js`: 恒星・星座データを管理する `AstroCatalog` クラスと登録 API。
- `core/`: プラネタリウム本体、時間、音声、設定レジストリ、シーンのライフサイクル。
- `systems/`: 星雲、流星、星座線、太陽/月/惑星、オーロラ、星軌跡などのレンダリングシステム。
- `ui/`: 画面ルーター、ホーム/モード選択、観測地セレクタ、イベントアーカイブ、撮影アルバム。
- `ui/library/`: 星座図書室の保存、表示モデル、詳細ビュー、本棚シーン、テクスチャ生成。
- `styles/`: 基本、ホーム、各画面、図書室、アルバムに分割したスタイル。
